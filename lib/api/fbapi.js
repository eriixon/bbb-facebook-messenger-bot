...
class CallFBApi {
 // Send message to Facebook API
  ...
  sendTextMessage(session, messageText) {
    ...
    return this.sendMessage(session.pid, {recipient:{id:session.fbid},message:{text:messageText,metadata:"TEXT"}});
  }
  // ask name of user
  askUserData(session){
    getToken(session.pid, PAGE_TOKEN => {
...
function getToken(id, callback){
  let db = new Datastore({filename : 'config/pages.db'});
  db.loadDatabase();
  db.find({ pageId: id}, (err, docs) => callback(docs[docs.length-1].token));
}
