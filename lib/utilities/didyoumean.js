    ...
    didYouMean(session) {
        ...
        bbb.createList(session, list => {
            if(!list || list.length===0) {
                ...
                fba.sendMessage(session.pid, {recipient:{id:session.fbid},message:{text:bph.NOT_FOUND,metadata:"TEXT"}});
            }else {
                ...
                if(newList.length!==0) fba.sendBusinessList(session,newList).then(()=>fba.sendTextMessage(session, bph.THANK_MESSAGE));
                else fba.sendMessage(session.pid, {recipient:{id:session.fbid},message:{text:bph.NOT_FOUND,metadata:"TEXT"}});
            };
        })
    }
};
