...
class LocationValidator {
    ...
    validateCity(userInput, session) {
        let self = this;
        let session = {pid:session.pid,fbid:session.fbid};// page and user IDs
        if(session.context.country === "USA") {
            usa.getCities(userInput, session.context.state, cityList => {
                ...
                else if (cityList.length > 4) fba.sendTextMessage(session, bph.MANY_CITIES);
                else self.createReplies(cityList, session);
            });
        } else if (session.context.country === "Canada") {
            // NEEDS API FOR CANADIAN CITIES
            fba.sendTextMessage(session, "FOR PRESENT IT WORKS ONLY FOR THE USA");
        }
    }

    // Create replies with cities
    createReplies (cityList, session) {
        let quickReplies = [];
