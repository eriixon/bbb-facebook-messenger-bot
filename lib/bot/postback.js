...
class Postbacks {
  ...
  handlePayload(event, session) {
    ...
    switch (payload) {
      // BUTTON Get start
      case 'GET_START':
        this.startConversation(session,(greetings)=>{fba.sendMessage(session.pid,session.token, greetings)});
        break;
      case 'FIND_BUSINESS':
        session.context = {};
        fba.sendMessage(session.pid,session.token, this.findBusinessMenu(session.fbid));
        break;
      case 'CHAT_AGENT':
        session.context = {};
        session.context.chat = true;
        fba.sendTextMessage(session,bph.CHAT_CALL);
        break;
