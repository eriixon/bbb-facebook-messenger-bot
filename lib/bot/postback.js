...
class Postbacks {
  ...
  handlePayload(event, session) {
    ...
    switch (payload) {
      // BUTTON Get start
      case 'GET_START':
        this.startConversation(sides,(greetings)=>{fba.sendMessage(sides.pid,greetings)});
        break;
      case 'FIND_BUSINESS':
        session.context = {};
        fba.sendMessage(sides.pid, this.findBusinessMenu(sides.uid));
        break;
      case 'CHAT_AGENT':
        session.context = {};
        session.context.chat = true;
        fba.sendTextMessage(sides,bph.AGENT_CHAT);
        break;
...
// Start topic for conversation
  startConversation(sides, callback) {
    fba.askUserData(sides).then(function(name){
      callback({
        recipient:{id: sides.uid},
        message:{
          attachment: {
            type: "template",
            payload:{
